...
- Optional inbound-reply capture: outgoing lead emails always send from one shared address, so a client's reply lands in that same shared inbox with no way for the assigned salesperson or the LMS to see it. A poller logs into that inbox over IMAP, matches each new message's `In-Reply-To`/`References` header against the `messageId` stored on the original `EmailLog` row to find the lead it belongs to, then (1) emails the assigned salesperson their own copy (a normal SMTP send to their real address — lands in their actual Outlook inbox, no Graph mailbox access needed) and (2) logs the reply against the lead as an `INBOUND` `EmailLog` row plus an `EMAIL_REPLY_RECEIVED` activity. Unmatched mail (not a reply to anything sent from the LMS) is left unread for manual review. Configured with `INBOUND_EMAIL_ENABLED`, `IMAP_HOST` (default `imap.gmail.com`), `IMAP_PORT` (default 993), `IMAP_USER`, `IMAP_PASS`, and `INBOUND_EMAIL_CHECK_INTERVAL_MINUTES` (default 5). Gmail requires an App Password for `IMAP_PASS` once 2-Step Verification is on (the normal account password won't work over IMAP) — the same app password already used for `SMTP_PASS` works here too.
